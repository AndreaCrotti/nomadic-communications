...
% Yes, I know it's unbelivable but there are some differences, for real! :)
	...
	802.11b protocol was born in October 1999, and since this date it has a discrete success. However, later on some other wireless devices, such as cordless telephones, use the same frequencies and therefore produce interferences with 802.11b-conforming devices and cause slower performance of these.
	% As a consequence,
	...
	\begin{table}[h!]
		\begin{tabularx}{15cm}{ | X | X | X | X | X | }
			...
			\hline
				CW & 310 $\nu$s & 310 $\nu$s & 139.5 $\nu$s & 139.5 $\nu$s \\
			\hline
				PLPC Preamble + PLPC Header & 96 $\nu$s & 96 $\nu$s & 26 $\nu$s & 26 $\nu$s \\
			\hline
				MAC Header + LLC & 52.36 $\nu$s & 26.18 $\nu$s & 48 $\nu$s & 5.33 $\nu$s \\
			\hline
				IP + UDP Headers & 40.73 $\nu$s & 20.36 $\nu$s & 37.33 $\nu$s & 4.15 $\nu$s \\
			\hline
				Data (1470 byte)& 2138.18 $\nu$s & 1069.09 $\nu$s & 1960 $\nu$s & 217.78 \\
			\hline
				TAIL DATA & -- & -- & 1 $\nu$s & 0.11 $\nu$s \\
			\hline
				ACK PLPC Preamble + Header & 96 $\nu$s & 96 $\nu$s & 26 $\nu$s & 26 $\nu$s \\
			\hline
				ACK & 20.36 $\nu$s & 10.18 $\nu$s & 18.67 $\nu$s & 2.07 $\nu$s \\
			\hline
				TAIL ACK & -- & -- & 1 $\nu$s & 0.25 $\nu$s \\
			...
			\hline
			\hline
				Theoretical physical throughput & 4.35 Mbit/s $\nu$s & 7.26 Mbit/s & 5.33 Mbit/s & 26.69 Mbit/s \\
			\hline
			\hline
